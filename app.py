...
from upload import get_supabase_admin, normalize_production, read_excel, upsert_production
...
with st.expander("📥 Drag & drop Excel file to update Supabase", expanded=False):
    if get_supabase_admin() is None:
        st.warning("Upload is disabled until you add `service_role_key` under `[supabase]` in Streamlit secrets. This key is required for secure database writes and must never be exposed in client-side code.")
    uploaded_file = st.file_uploader("Upload production Excel", type=["xlsx"], accept_multiple_files=False, key="production_uploader")
    if uploaded_file is not None and get_supabase_admin() is not None:
        try:
            sheets = read_excel(uploaded_file)
            sheet_name = st.selectbox("Excel sheet", list(sheets.keys()), key="production_sheet")
            normalized, validation_errors = normalize_production(sheets[sheet_name])
            st.caption(f"Detected {len(sheets[sheet_name]):,} source rows → {len(normalized):,} valid rows after cleaning.")
            ...
            if not normalized.empty:
                ...
                if not validation_errors:
                    from upload import find_existing_keys
                    existing_keys = find_existing_keys(normalized)
                    new_count = sum((str(r["date"])[:10], str(r["ALIAS"]).strip()) not in existing_keys for r in normalized.to_dict("records"))
                    update_count = len(normalized) - new_count
                    m1, m2, m3 = st.columns(3)
                    m1.metric("Rows ready", f"{len(normalized):,}")
                    m2.metric("New rows", f"{new_count:,}")
                    m3.metric("Updates", f"{update_count:,}")
                    st.info("Existing date + ALIAS records will be updated; new keys will be inserted. The database has a unique constraint on date + ALIAS to prevent duplicates.")
                    confirm = st.checkbox("I have reviewed the preview and want to update Supabase.", key="production_upload_confirm")
                    if st.button("Upload to Supabase", type="primary", disabled=not confirm, key="production_upload_button"):
                        try:
                            result = upsert_production(normalized, uploaded_file.name)
                            st.success(f"Upload complete: {result['inserted']:,} inserted, {result['updated']:,} updated.")
                            st.cache_data.clear()
                            st.rerun()
                        except Exception as exc:
