...
def render_wds_uploader():
    with st.expander("📚 Drag and Drop Excel WDS", expanded=False):
        st.caption("Drop multiple daily WDS reports here. The ETL extracts Date, Well, BO, BW, Gas, Total Injeksi bbls, and the daily Actual Production value, converts names such as M # 01 → M-01, and can upload the cleaned result to Supabase.")
        ...
        if st.button("Upload extracted data to Supabase", type="primary", disabled=not confirm, key="upload_wds_button"):
            try:
                ...
                st.cache_data.clear()
                st.success(f"Successfully uploaded {rows:,} date + well records, Total Injeksi bbls values, and the Actual Production daily totals. Blank injection values did not overwrite existing injection_rate data.")
                st.rerun()
