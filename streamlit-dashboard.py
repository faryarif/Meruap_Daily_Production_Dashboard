...
# ETL — WRITE + VALIDATE
# ----------------------------------------------------------------------------
def write_ProdWellBasis(df: pd.DataFrame):
    client = get_supabase()
    ...
    client.table("ProdWellBasis").insert(clean_rows).execute()
    ...
    if uploaded:
        try:
            ...
            if errors:
                ...
                try:
                    hist_resp = get_supabase().table("ProdWellBasis").select("date").execute()
                ...
                if st.button("📤 Append to Supabase", type="primary", use_container_width=True):
                    try:
                        write_ProdWellBasis(raw[DATA_COLS])
                        st.cache_data.clear()
